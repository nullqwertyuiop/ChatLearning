    ...
    def loadPlugins(self):
        ...
        for filename in os.listdir("Clplugin"):
            if (
                ...
                or filename == "simuse.py"
            ):
                continue
            print(filename, "插件已加载")
            threads = threading.Thread(
                target=self.runPlugin, args=(filename,)
            )  # 多线程并行载入插件
            threads.start()  # 运行
            ...
            if self.fuzzyfinder(self.command) != [] and self.command != "":
                ...
                if fromchat != 0:
                    simuse.Send_Message(
                        ...
                        fromchat,
                        2,
                        '未知指令"{}"   你是否想输入以下指令？'.format(self.command),
                        1,
                    )
                ...
                if fromchat != 0:
                    simuse.Send_Message(
                        ...
                        fromchat,
                        2,
                        '未知指令"{}"   请输入help/?查看帮助'.format(self.command),
                        1,
                    )
        except:
            print('<-未知指令"{}"   请输入help/?查看帮助'.format(self.command))
    ...
    def run(self):
        if self._target:
            try:
                ...
                print("{}：正在进行第{}次重试".format(self._target.__name__, self.Trynum + 1))
                time.sleep(2)
