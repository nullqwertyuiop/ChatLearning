...
def restart(version):
    ...
    if system == "Windows":
        ...
        linux_command = json_load(open("command.json", "r", encoding="utf-8-sig"))
        ...
        TempList += "echo 更新完成，正在启动...\n"
        TempList += per_command.format("ChatLearning_v{}".format(version)) + "\n"
        # TempList += "timeout /t 3 /nobreak\n"
...
def ReplyWait(args: str, fromchat=0):
    ...
    if fromchat != 0:
        simuse.Send_Message(
            ...
            fromchat,
            2,
            "已将回复等待时间设置为{:g}±{:g}秒".format(BenchmarkTime, RandomTime),
            1,
        )
...
def GetAutoTask():
    try:
        ...
        Taskdict = {}
        pickle_dump(Taskdict, open("AutoTask/AutoTask.clc", "wb"))
    return Taskdict
...
def sendgroupmessage(arglist):
    ...
    sendstr = sendstr.replace("{month}", datetime.datetime.now().strftime("%m"))
...
def SetTempMessage(data, num, fromchat=0):
    ...
    if fromchat != 0:
        simuse.Send_Message(data, fromchat, 2, "每个群的消息缓存已设置为{}条".format(num), 1)
...
def setadmin(adminnum, fromchat=0):
    ...
    if fromchat != 0:
        simuse.Send_Message(data, fromchat, 2, "管理员QQ号已设置为" + str(adminlist), 1)
...
def grouplist(fromchat=0):
    ...
    if fromchat != 0:
        sendtext = (
            ...
            + "\n"
            + "已开启记录的群："
            + str(learninggrouplist)
            + "\n"
            + "已开启答复的群："
            + str(replygrouplist)
            + "\n"
            + "允许自主管理的群："
            + str(list(subadmindict.keys()))
            + "\n"
            + "不录入总词库的群："
            + str(unmergegrouplist)
        )
        simuse.Send_Message(data, fromchat, 2, sendtext, 1)
...
def learninginterval(interval, fromchat=0):
    ...
    if fromchat != 0:
        simuse.Send_Message(data, fromchat, 2, "已设置词库链学习间隔时间" + str(interval) + "秒", 1)
    file = open("config.clc", "r", encoding="utf-8-sig")
    config = json_load(file)
    file.close()
    config["interval"] = interval
    file2 = open("config.clc", "w", encoding="utf-8-sig")
    json_dump(config, file2, indent=3, ensure_ascii=False)
...
def replychance(chance, fromchat=0):
    ...
    if chance[: chance.find(" ")] == "-s":
        ...
        if fromchat != 0:
            simuse.Send_Message(data, fromchat, 2, "已设置回复的触发概率" + str(chance) + "%", 1)
        file = open("config.clc", "r", encoding="utf-8-sig")
        config = json_load(file)
        file.close()
...
def voicereplychance(chance, fromchat=0):
    ...
    if chance[: chance.find(" ")] == "-s":
        ...
        if fromchat != 0:
            simuse.Send_Message(
                data, fromchat, 2, "已设置语音回复的触发概率" + str(chance) + "%", 1
            )
        file = open("config.clc", "r", encoding="utf-8-sig")
        config = json_load(file)
        file.close()
...
def addgroup(args, fromchat=0):
    ...
    if args[:9] == "learning " or args[:10] == "learnings ":
        ...
        try:
            ...
            if fromchat != 0:
                simuse.Send_Message(
                    data, fromchat, 2, "词库合并错误，请关闭Learning/Reply后重新操作", 1
                )
        ...
        try:
            ...
            if fromchat != 0:
                simuse.Send_Message(data, fromchat, 2, "未找到文件,请检查是否在AutoTask目录下", 1)
            return None
        except UnicodeDecodeError:
            print("读取错误，请检查文件编码是否为UTF-8")
            if fromchat != 0:
                simuse.Send_Message(data, fromchat, 2, "读取错误，请检查文件编码是否为UTF-8", 1)
            return None
        if Task.CheckTask() == 0:
            print(Task.ErrorTips)
            if fromchat != 0:
...
def removegroup(args, fromchat=0):
    ...
    if args[:9] == "learning ":
        ...
        try:
            ...
            if fromchat != 0:
                simuse.Send_Message(
                    data, fromchat, 2, "删除失败，未找到名为“{}”的定时任务".format(TaskName), 1
                )
            return None
...
def learning(learningsign, mergesign, fromchat=0):
    global data, learning_t, merge_t
    if learningsign == 0 and mergesign == 0:
        ...
        learning_t = My_Thread(target=ChatLearning.main)
        merge_t = My_Thread(target=ChatMerge.main)
        learningsign = 1
        mergesign = 1
        learning_t.start()
        merge_t.start()
...
def reply(replysign, fromchat=0):
    ...
    if replysign == 0:
        ...
        reply_t = My_Thread(target=ChatReply.main)
        replysign = 1
        reply_t.start()
        return replysign
    else:
...
def merge(time, fromchat=0):
    ...
    if fromchat != 0:
        simuse.Send_Message(data, fromchat, 2, "已设置总词库更新时间" + str(time) + "秒", 1)
    file = open("config.clc", "r", encoding="utf-8-sig")
    config = json_load(file)
    file.close()
...
def typefreq(data, args, fromchat=0):
    ...
    if fromchat != 0:
        simuse.Send_Message(
            data, fromchat, 2, "已设置{}回复阈值".format(type) + str(freq) + "次", 1
        )
...
async def getcommand_tui():
    global listen_t, subadmin_t, autotask_t
    listen_t = My_Thread(target=getcommand_chat)
    listen_t.start()
    subadmin_t = My_Thread(target=ChatSubadmin.main)
    subadmin_t.start()
    autotask_t = My_Thread(target=RunAutoTask)
    autotask_t.start()
...
if __name__ == "__main__":
    # 更新判断
    global my_name, system, update_sign
    update_sign = False
    my_path = sys.argv[0]
    ...
    if data["Key"] != "":
        try:
            ...
            print(
                "取得session失败，但已与api-http取得连接(host,port正常)。\n可能的问题：\n1.Key不正确\n2.api-http出现异常"
            )
            os.system("pause")
            exit()
    if learningsign == 1:
