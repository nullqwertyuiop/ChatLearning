...
def replyanswer(data, sender, filterlist, Accuratedict=None):  # 发送答案
    ...
    simuse.Send_Message(
        data, sender, 2, "条目共有" + str(len(filterlist)) + "个" + "\n" + tips, 1
    )
    nodedict = {
        "senderId": data["qq"],
        "time": int(time.time()),
    ...
    for i in filterlist:
        ...
        for k in templist:
            ...
            try:
                if k["type"] == "At":
                    changelist = [
                        {"type": "Plain", "text": "该条目为@消息，要显示完整请在ChatLearning控制台查看"}
                    ]
                    # print(k)
                    # print('-',changelist)
                elif k["type"] == "Quote":
                    changelist = [
                        {"type": "Plain", "text": "该条目为回复类消息，要显示完整请在ChatLearning控制台查看"}
                    ]
                    # print(k)
                    # print('-',changelist)
                elif k["type"] == "AtAll":
                    changelist = [
                        {"type": "Plain", "text": "该条目为@全员消息，要显示完整请在ChatLearning控制台查看"}
                    ]
                    # print(k)
                    # print('-',changelist)
                elif k["type"] == "Poke":
                    changelist = [
                        {"type": "Plain", "text": "该条目为戳一戳类消息，要显示完整请在ChatLearning控制台查看"}
                    ]
            except:
                # print('error')
                pass
        ...
        if Accuratedict != None:
            index["text"] = "\n标记:{}\n精确匹配:{}".format(
                str(filterlist.index(i)), accurate_tip
            )
        else:
...
def getnode(data, Filterconfig, filterlist, sender, Accuratedict=None):
    ...
    if templist != []:
        if sendtext != "":
            ...
            simuse.Send_Message(
                data, sender, 2, "删除" + str(len(templist)) + "个条目成功！", 1
            )
...
def sensitivecheck(question, sender, group):
    ...
    if str(question) in Filterconfig["sensitive"]:
        creatblack(sender)
        print(
            "已过滤，原因：与敏感问题匹配，已将发送者加入黑名单", "发送者{}".format(sender), "来自群{}".format(group)
        )
        return 0
    else:
        for i in question:
...
def filtercheck(question, sender="|回复过滤", group="|回复过滤", display=True):
    ...
    if str(question) in Filterconfig["filter"]:
        if display == True:
            print("已过滤，原因：与过滤字典精确匹配", "发送者{}".format(sender), "来自群{}".format(group))
        return 0
