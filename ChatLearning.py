    ...
    while 1:
        ...
        for i in textdict:  # 遍历群号
            ...
            for k in textdict[i]:  # 开始记录问题
                ...
                messagechain.pop(0)
                if (
                    messageinfo["time"] - messagesign["signtime"] > interval
                ):  # 若相同群，收集到的两个消息的间隔大于900秒，则新的消息重新标记“1”
...
def custom_answer(data, fromchat):
    ...
    while True:
        ...
        try:
            ...
            time.sleep(0.2)
            simuse.Send_Message(data, fromchat, 2, "该问题是否为正则表达式?\n0.否\n1.是", 1)
            while True:
        ...
        while True:
            ...
            time.sleep(0.5)
            simuse.Send_Message(
                data, fromchat, 2, "请输入答案的权重(这是一个非负的整数,越大表明抽中该答案的概率越大)", 1
            )
            while True:
                while True:
