...
def createxcel(groupcldict):
    ...
    for groupnum in list(groupcldict.keys()):
        ...
        for question in questionlist:
            ...
            question_time = time.strftime("%Y-%m-%d %H:%M:%S", timeArray_question)
            question_info = ""
            ...
            for question_messagechain in question:
                if question_messagechain["type"] == "Plain":
                    ...
                    question_info = question_info + question_messagechain["text"]
                elif question_messagechain["type"] == "Image":
                    question_imagesign = 1
                    question_info = (
                        question_info + question_messagechain["imageId"]
                    )  ######
            ...
            for answerdict in answerlist:
                ...
                answer_time = time.strftime("%Y-%m-%d %H:%M:%S", timeArray_answer)
                answer_info = ""
                for answer_messagechain in eval(answerdict["answertext"]):
                    if answer_messagechain["type"] == "Plain":
                        ...
                        answer_info = answer_info + answer_messagechain["url"]  ######
                ...
                try:
                    ...
                    sheet.write(sheet_line, 2, question_info, style=setstyle_text)
                    ...
                    if answer_imagesign == 1:
                        ...
                        sheet.write(
                            sheet_line, 4, xlwt.Formula(link), style=setstyle_text
                        )
                    else:
                        sheet.write(sheet_line, 4, answer_info, style=setstyle_text)
...
def uploadcos(data, filename):
    ...
    response_upload = client.upload_file(
        Bucket=Bucket_name,
        LocalFilePath=filename + ".xls",
        Key=data["qq"] + "-" + filename + ".xls",
        ACL="public-read",
        EnableMD5=False,
    )
...
def delcl(data, sender, lines_dict, nodelist, target_type=0, group=0):
    ...
    while 1:
        ...
        if command != None:
            ...
            if errorlines == 0 and seerrorlines == 0:
                ...
                print("行数错误，请重新输入\n结束行应大于起始行")
                simuse.Send_Message(
                    data, target, target_type, "行数错误，请重新输入\n结束行应大于起始行", 1
                )
    ...
    for group in changedict:
        cldict_orign = pickle_load(open("WordStock/" + str(group) + ".cl", "rb"))
        ...
        for nodedict in changelist:
            ...
            if answerlist == []:
                cldict.pop(question)
        cldict_orign = cldict
        pickle_dump(cldict_orign, open("WordStock/" + str(group) + ".cl", "wb"))
    if changedict != {} and nofoundlines == []:
        ...
        simuse.Send_Message(
            data, target, target_type, "删除{}个条目成功\n未找到行{}".format(delsign, tips), 1
        )
    else:
        print("删除失败")
...
# 查找问题
def findallquestion(
    data, sender, cllist, question, allquestion=0, group=0, target_type=0
):
    if group != 0:
        target = group
    else:
                ...
                if k["type"] == "Plain":
                    if str(j).find(k["text"]) != -1 and allquestion == 0:
                        ...
                        if answerlist == []:
                            answerlist = [
                                {
                                    "answertext": "[{'type':'Plain','text':'该问题无答案'}]",
                                    "time": 100,
                                }
                            ]
                        for answerdict in answerlist:
    ...
    while 1:
        ...
        if command == str(1):
            ...
            time.sleep(1)
            delcl(
                data, sender, lines_dict, nodelist, target_type=target_type, group=group
            )
            break
        elif command == str(2):
            os.remove(filename + ".xls")
...
def findallcontrol(data, sender, group=0):
    ...
    while 1:
        ...
        if command == str(1):
            ...
            while 1:
                time.sleep(0.5)
                question = ChatAdmin.get_admin_question(
                    data, sender=sender, group=group
                )
                if question != None:
                    if (
                        findallquestion(
            ...
            while 1:
                ...
                if question != None:
                    if (
                        findallanswer(
                            data, sender, cllist, question, group, target_type
                        )
                        == 2
                    ):
                        simuse.Send_Message(data, target, target_type, "请发送关键字", 1)
                ...
                if choicegroup != None:
                    ...
                    if choicegroup in grouplist:
                        ...
                        findallquestion(
                            ...
                            sender,
                            onecllist,
                            question=[{"type": "Plain", "text": "AllFind"}],
                            allquestion=1,
                            group=group,
                            target_type=target_type,
                        )
