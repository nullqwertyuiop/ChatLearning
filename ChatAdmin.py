...
def get_admin_question(data, sender, group=0):
    ...
    for i in message:
        if group == 0:
            if i["type"] == "FriendMessage" and i["sender"] == sender:  # 判断监听到的消息是否为群消息
                messagechain = i["messagechain"]
                messagechain.pop(0)
                question = messagechain
                return question
        elif group != 0:
            if i["type"] == "GroupMessage" and i["sender"] == sender:  # 判断监听到的消息是否为群消息
                messagechain = i["messagechain"]
                messagechain.pop(0)
                question = messagechain
                return question
...
def get_admin_command(data, adminlist=0, sender=0, group=0):
    ...
    for i in message:
        if sender != 0 and group == 0:
            ...
            if (
                i["type"] == "FriendMessage" and i["sender"] in adminlist
            ):  # 判断监听到的消息是否为群消息
                messagechain = i["messagechain"]
                command = messagechain[1]
                if command["type"] == "Plain":
...
def getnode(data, tempdict, answerlist, group, sender, question):
    ...
    if templist != []:
        if sendtext != "":
            ...
            simuse.Send_Message(
                data, sender, 2, "删除" + str(len(templist)) + "个答案成功！", 1
            )
            if addfilter != 0:
                time.sleep(1)
                simuse.Send_Message(data, sender, 2, "已添加入过滤列表", 1)
...
def replyanswer(data, sender, answer, questiondict):  # 发送答案
    ...
    simuse.Send_Message(
        data, sender, 2, "找到" + str(len(answer)) + "个答案" + "\n" + tips, 1
    )
    nodedict = {
        "senderId": data["qq"],
    ...
    for i in answer:
        ...
        for k in templist:
            ...
            try:
                if k["type"] == "At":
                    changelist = [
                        {"type": "Plain", "text": "该答案为@消息，要显示完整请在ChatLearning控制台查看"}
                    ]
                    # print(k)
                    # print('-',changelist)
                elif k["type"] == "Quote":
                    changelist = [
                        {"type": "Plain", "text": "该答案为回复类消息，要显示完整请在ChatLearning控制台查看"}
                    ]
                    # print(k)
                    # print('-',changelist)
                elif k["type"] == "AtAll":
                    changelist = [
                        {"type": "Plain", "text": "该答案为@全员消息，要显示完整请在ChatLearning控制台查看"}
                    ]
                    # print(k)
                    # print('-',changelist)
                elif k["type"] == "Poke":
                    changelist = [
                        {"type": "Plain", "text": "该答案为戳一戳类消息，要显示完整请在ChatLearning控制台查看"}
                    ]
            except:
                # print('error')
                pass
...
def main(data, adminlist, group, fromchat):
    ...
    if not (group in getfilelist()):
        ...
        if fromchat != 0:
            simuse.Send_Message(data, fromchat, 2, "群定位失败，未找到群" + str(group), 1)
        return None
    print("<-定位到群", group)
    if fromchat != 0:
        tips = "定位到群" + str(group) + "\n" + "请发送“问题”，发送admin可退出"
